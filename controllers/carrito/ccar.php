...
switch ($action) {
    ...
    case 'checkout':
        ...
        try {
            ...
            $insDetalle = $pdo->prepare('INSERT INTO detalle_pedido (pedido_id, producto_id, cantidad, precio, notas) VALUES (?,?,?,?,?)');
            $prodStmt = $pdo->prepare('SELECT id, precio, disponible FROM productos WHERE id=?');
            $subtotal = 0.0;

            foreach ($_SESSION['cart'] as $pid => $item) {
                $cantidad = max(1, (int)($item['cantidad'] ?? 1));
                $prodStmt->execute([(int)$pid]);
                ...
                $insDetalle->execute([$pedido_id, (int)$pid, $cantidad, $precioActual, null]);
