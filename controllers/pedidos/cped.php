...
require_login();
require_role(['admin', 'mesero', 'cajero', 'cliente']);

$action = $_GET['a'] ?? 'list';

// SEGURIDAD: Estados de pedido permitidos
const ESTADOS_PEDIDO = ['pendiente', 'en_preparacion', 'listo', 'entregado', 'completado', 'cancelado'];
const ESTADOS_PEDIDO_ACTIVOS = ['pendiente', 'en_preparacion', 'listo', 'entregado'];

switch ($action) {
    case 'list':
        // Listar pedidos con filtros opcionales
        $estado_filtro = $_GET['estado'] ?? null;
        $isCliente = has_role('cliente');
    ...
    case 'create':
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($mesa_id <= 0) {
                $_SESSION['error'] = 'Debes seleccionar una mesa disponible para crear el pedido.';
                header('Location: cped.php?a=create');
                exit;
            }
            ...
            // Validar mesa
            if ($mesa_id) {
                $stmt = $pdo->prepare('SELECT id, estado FROM mesas WHERE id=? FOR UPDATE');
    ...
    case 'detalle':
        ...
        $stmt = $pdo->prepare('SELECT p.*, u.nombre as usuario_nombre, m.numero as mesa_numero 
                               ...
                               LEFT JOIN usuarios u ON p.usuario_id = u.id 
                               LEFT JOIN mesas m ON p.mesa_id = m.id
                               WHERE p.id=?' . ($isCliente ? ' AND p.usuario_id=?' : ''));
        $params = $isCliente ? [$id, (int)$_SESSION['id']] : [$id];
        $stmt->execute($params);
    ...
    case 'agregar_producto':
        ...
        $stmt = $pdo->prepare('SELECT precio, disponible FROM productos WHERE id=?');
    ...
    case 'actualizar_estado':
        ...
        try {
            ...
            if (in_array($estado, ['completado', 'cancelado'], true) && $pedido['mesa_id']) {
                ...
                $stmt = $pdo->prepare('SELECT COUNT(*) FROM pedidos WHERE mesa_id=? AND id<>? AND estado IN ("pendiente","en_preparacion","listo","entregado")');
    ...
    case 'actualizar_mesa':
        ...
        if ($estado === 'disponible') {
            $check = $pdo->prepare('SELECT COUNT(*) FROM pedidos WHERE mesa_id=? AND estado IN ("pendiente","en_preparacion","listo","entregado")');
            $check->execute([$mesaId]);
            if ((int)$check->fetchColumn() > 0) {
                http_response_code(409);
                header('Content-Type: application/json');
                echo json_encode(['ok' => false, 'error' => 'No se puede liberar la mesa porque tiene pedidos activos']);
                exit;
            }
        }
